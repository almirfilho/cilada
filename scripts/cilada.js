...
(function() {
  var $canvas, Ball, Hole, Mallandro, Wall, audio, b2Body, b2BodyDef, b2CircleShape, b2ContactListener, b2DebugDraw, b2Fixture, b2FixtureDef, b2PolygonShape, b2Vec2, b2World, ball, beginGame, bodyDef, config, contact, ctx, endGame, fixDef, game, hasAccelerationDevice, hasAccelerationSupport, hasAccelerometer, holes, init, loadSounds, mallandro, update, walls, world;
  ...
  init = function() {
    ...
    setTimeout(function() {
      if (!hasAccelerationSupport) {
        $('#prompt p, #prompt button').remove();
        return $('#prompt').append('<p class="no-support"><strong>Pôôo meu irmão!!</strong><br />Seu computador não tem <strong>Acelerômetro</strong>!<br />Buy a mac ;)</p>');
      }
    }, 20);
    ...
    if ((window.DeviceMotionEvent != null) && !orientation) {
      window.addEventListener('devicemotion', function(event) {
        ...
        return ball.impulse.y = event.accelerationIncludingGravity.y / config.scale * 3;
      });
    }
  ...
  hasAccelerometer = function() {
    ...
    if (window.DeviceOrientationEvent != null) {
      window.addEventListener('deviceorientation', function(orientData) {
        return hasAccelerationDevice = true;
      });
    }
